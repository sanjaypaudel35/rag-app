...
@section("content")
    ...
    <form method="POST" action="{{ route("password.update", ["project_slug" => request()->route("project_slug")]) }}">
        @csrf
        ...
        <div>
            ...
            <input id="email" class="block mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-blue-500 dark:focus:border-blue-600 focus:ring-blue-500 dark:focus:ring-blue-600 rounded-md shadow-sm" type="email" name="email" value="{{ old("email", $request->email) }}" required autofocus autocomplete="username" />
            @error("email")
